...
echo "Submitting fast debug fixed-init training job with PIPE_TAG=$PIPE_TAG"
TRAIN_JOB_ID=$(
  sbatch --parsable \
    --partition=debug \
    --time=12:00:00 \
    --cpus-per-task=16 \
    --mem=64G \
    --export=ALL,PIPE_TAG="$PIPE_TAG",RUN_NAME="fixedinit_offset_fast12h_${PIPE_TAG}",FIXED_INIT_PROFILE=offset_ref,FIXED_INIT_POS_JITTER_STD=0.0,FIXED_INIT_VEL_JITTER_STD=0.0,FIXED_INIT_JITTER_TRIES=1,UPDATES=1000,NUM_ENVS=16,ROLLOUT_STEPS=96,PPO_EPOCHS=4,MINIBATCH_SIZE=512,EVAL_EVERY=20,EVAL_EPISODES=1,VEC_ENV=subproc,MP_START_METHOD=spawn,HORIZON_STEPS=360,MAX_ACTION_NORM=2.0,INITIAL_LOG_STD=-4.0,ENT_COEF=0.0,REWARD_SCALE=900.0,ESCAPE_RADIUS=8.0,W_POS=10.0,W_VEL=7.0,W_FUEL=0.00005,W_NEAR_COLLISION=8.0,W_COLLISION=300.0,W_ESCAPE=20.0,W_SWITCH=0.15,W_PHASE=0.18,EVAL_POS_THRESHOLD_TRAIN=0.06,EVAL_VEL_THRESHOLD_TRAIN=0.09,EVAL_CONSECUTIVE_CONVERGED_TRAIN=220,EVAL_MIN_TOTAL_STEPS_TRAIN=300,SAVE_TOPK=3,EARLY_STOP_ON_STRICT_SUCCESS=1,EARLY_STOP_SUCCESS_RATE=1.0,EARLY_STOP_MAX_FAILURE_RATE=0.0,EARLY_STOP_PATIENCE_EVALS=1,EARLY_STOP_MIN_EVALS=1 \
    sbatch_train_fixed_init_quick_30m.sh
)

echo "Submitting dependent fast debug eval job afterok:$TRAIN_JOB_ID"
EVAL_JOB_ID=$(
  sbatch --parsable \
    --partition=debug \
    --time=02:00:00 \
