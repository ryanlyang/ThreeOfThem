...
PIPE_TAG="${PIPE_TAG:-mpc_olws_compare_$(date +%Y%m%d_%H%M%S)}"

echo "Submitting MPC open-loop + warm-start compare job with PIPE_TAG=$PIPE_TAG"
JOB_ID=$(
  sbatch --parsable \
    --partition=tier3 \
    --time=06:00:00 \
