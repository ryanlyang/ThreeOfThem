...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MPC/iLQR convergence run with (1) open-loop trajectory optimization and (2) warm-started MPC."
    )
    ...
    p.add_argument("--jitter-setups", type=int, default=10)
    p.add_argument("--jitter-pos-std", type=float, default=0.006)
    p.add_argument("--jitter-vel-std", type=float, default=0.004)
    p.add_argument("--jitter-tries", type=int, default=64)
    ...
    p.add_argument("--openloop-cem-iters", type=int, default=12)
    p.add_argument("--openloop-cem-population", type=int, default=128)
    p.add_argument("--openloop-cem-elite-frac", type=float, default=0.12)
    p.add_argument("--openloop-cem-init-std-frac", type=float, default=0.55)
    p.add_argument("--openloop-cem-min-std-frac", type=float, default=0.05)
    p.add_argument("--openloop-cem-smoothing", type=float, default=0.25)
    p.add_argument("--openloop-exec-steps", type=int, default=40)
    ...
    p.add_argument("--trail-len", type=int, default=120)
    p.add_argument("--frame-stride", type=int, default=1)
    p.add_argument("--axis-pad", type=float, default=0.25)
    p.add_argument("--outdir", type=str, default="inference_mpc_ilqr_openloop_warmstart")
    return p.parse_args()


def make_env_config(args: argparse.Namespace, *, pos_jitter_std: float, vel_jitter_std: float, jitter_tries: int) -> EnvConfig:
    ...
    return EnvConfig(
        ...
        fixed_init_pos_jitter_std=pos_jitter_std,
        fixed_init_vel_jitter_std=vel_jitter_std,
        fixed_init_jitter_tries=jitter_tries,
    )
...
def make_ilqr_config(args: argparse.Namespace) -> ILQRConfig:
    return ILQRConfig(
        ...
        cem_iters=args.openloop_cem_iters,
        cem_population=args.openloop_cem_population,
        cem_elite_frac=args.openloop_cem_elite_frac,
        cem_init_std_frac=args.openloop_cem_init_std_frac,
        cem_min_std_frac=args.openloop_cem_min_std_frac,
        cem_smoothing=args.openloop_cem_smoothing,
        cem_seed=args.seed,
    )
...
def run_one_setup(
    ...
    openloop = controller.optimize_open_loop_sequence(
        x0=x,
        phase_idx=phase_idx,
    ...
    openloop_dt = time.perf_counter() - t0
    warm_actions = openloop.actions.copy()
    ...
    while not done and t < int(args.max_steps):
        if t < max(0, int(args.openloop_exec_steps)):
            action_vec = warm_actions[0].copy()
            warm_actions = controller.shift_actions(warm_actions)
        else:
            ...
            if need_replan:
                ...
                planning_time_sum += time.perf_counter() - t_plan
                planning_calls += 1
        ...
        if (t % max(1, int(args.log_every)) == 0) or done or (t == int(args.max_steps)):
            print(
                f"[{label} seed={setup_seed} step={t:4d}] pos_err={pos_err:.5f} vel_err={vel_err:.5f} "
    ...
    if len(pos_err_hist) == 0:
        ...
        min_pos_step = int(args.max_steps) + 1
    else:
    ...
    rank_tuple = [has_converged, has_failure, rank_step, -int(max_converge_streak), final_pos_err, final_vel_err, min_pos_err, min_pos_step]
    ...
    return {
        ...
        "openloop_cost": float(openloop.total_cost),
        "openloop_time_sec": float(openloop_dt),
...
def main() -> None:
    ...
    base_cfg = make_env_config(args, pos_jitter_std=0.0, vel_jitter_std=0.0, jitter_tries=1)
    jitter_cfg = make_env_config(
        args,
        pos_jitter_std=float(args.jitter_pos_std),
        vel_jitter_std=float(args.jitter_vel_std),
        jitter_tries=max(1, int(args.jitter_tries)),
    )
    match_weights = make_match_weights(args)
    ilqr_cfg = make_ilqr_config(args)

    rng = np.random.default_rng(args.seed)
    base_seed = int(rng.integers(1, 2**31 - 1))
    jitter_seeds = [int(rng.integers(1, 2**31 - 1)) for _ in range(max(0, int(args.jitter_setups)))]

    runs = []
    print("=== exact training setup (no jitter) ===", flush=True)
    runs.append(run_one_setup(base_seed, base_cfg, match_weights, ilqr_cfg, args, label="exact"))

    for i, s in enumerate(jitter_seeds):
        print(f"=== jitter setup {i + 1}/{len(jitter_seeds)} ===", flush=True)
        runs.append(run_one_setup(s, jitter_cfg, match_weights, ilqr_cfg, args, label=f"jitter_{i:02d}"))

    reference_path = Figure8ChoreographyEnv(config=base_cfg, weights=match_weights).reference.positions
    for r in runs:
        gif_path = out_dir / f"{r['label']}_seed_{r['setup_seed']}.gif"
        save_rollout_gif(
            positions_hist=r["positions_hist"],
            reference_path=reference_path,
            out_gif=gif_path,
            title=f"{r['label']} (seed={r['setup_seed']})",
    ...
    best_name = "best_convergence" if bool(best["strict_converged"]) else "best_attempt_not_converged"
    best_gif = out_dir / f"{best_name}_{best['label']}_seed_{best['setup_seed']}.gif"
    save_rollout_gif(
        positions_hist=best["positions_hist"],
        reference_path=reference_path,
        out_gif=best_gif,
        title=f"{best_name} ({best['label']}, seed={best['setup_seed']})",
        trail_len=args.trail_len,
        frame_stride=args.frame_stride,
        axis_pad=args.axis_pad,
    )

    exact_runs = [r for r in runs if r["label"] == "exact"]
    jitter_runs = [r for r in runs if r["label"].startswith("jitter_")]
    summary_rows = [{k: v for k, v in r.items() if k not in {"positions_hist", "pos_err_hist", "vel_err_hist", "phase_hist", "last_info"}} for r in runs]

    summary = {
        "controller": "mpc_ilqr_openloop_warmstart",
        "strategy": ["open_loop_trajectory_optimization", "warm_started_mpc_ilqr"],
        "strict_criteria": {
        ...
        "exact_setup_seed": base_seed,
        "jitter_setup_seeds": jitter_seeds,
        "jitter_config": {
            "pos_std": float(args.jitter_pos_std),
            "vel_std": float(args.jitter_vel_std),
            "tries": int(args.jitter_tries),
        },
        "best_index": int(best_idx),
        "best_label": str(best["label"]),
        "best_seed": int(best["setup_seed"]),
        "best_strict_converged": bool(best["strict_converged"]),
        "best_rank_tuple": best["rank_tuple"],
        "best_gif": str(best_gif),
        "comparison": {
            "exact_strict_success_rate": float(np.mean([float(r["strict_converged"]) for r in exact_runs])) if exact_runs else float("nan"),
            "jitter_strict_success_rate": float(np.mean([float(r["strict_converged"]) for r in jitter_runs])) if jitter_runs else float("nan"),
            "exact_collision_rate": float(np.mean([float(r["collided"]) for r in exact_runs])) if exact_runs else float("nan"),
            "jitter_collision_rate": float(np.mean([float(r["collided"]) for r in jitter_runs])) if jitter_runs else float("nan"),
        },
        "runs": summary_rows,
        "base_env_config": asdict(base_cfg),
        "jitter_env_config": asdict(jitter_cfg),
    ...
    print(f"saved {len(runs)} gifs under: {out_dir}", flush=True)
